...
class Recognizer:
  ...
  def recognition(self, audio, *args, **kwargs):
    if self.type == RecognizerTypes.SPHINX:
      return self._sphrinx(audio, *args, **kwargs)
    if self.type == RecognizerTypes.GOOGLE_SPEECH_RECOGNITION:
      return self._google_speech(audio, *args, **kwargs)
    if self.type == RecognizerTypes.GOOGLE_CLOUD_SPEECH:
      return self._google_cloud_speech(audio, *args, **kwargs)
    if self.type == RecognizerTypes.WIT_AI_KEY:
      return self._wit_ai_key(audio, *args, **kwargs)
    if self.type == RecognizerTypes.MICROSOFT_BING_VOICE_RECOGNITION:
      return self._microsoft_bing(audio, *args, **kwargs)
    if self.type == RecognizerTypes.HOUNDIFY:
      return self._houndify(audio, *args, **kwargs)
    if self.type == RecognizerTypes.IBM_SPEECH_TO_TEXT:
      return self._ibm(audio, *args, **kwargs)
  ...
  def _google_cloud_speech(self, audio):
    ...
    try:
      print("Google Cloud Speech thinks you said " + self.recognizer.recognize_google_cloud(audio, credentials_json=GOOGLE_CLOUD_SPEECH_CREDENTIALS))
    except sr.UnknownValueError:
  ...
  def _wit_ai_key(self, audio):
    ...
    try:
      print("Wit.ai thinks you said " + self.recognizer.recognize_wit(audio, key=WIT_AI_KEY))
    except sr.UnknownValueError:
  ...
  def _microsoft_bing(self, audio):
    ...
    try:
      print("Microsoft Bing Voice Recognition thinks you said " + self.recognizer.recognize_bing(audio, key=BING_KEY))
    except sr.UnknownValueError:
  ...
  def _houndify(self, audio):
    ...
    try:
      print("Houndify thinks you said " + self.recognizer.recognize_houndify(audio, client_id=HOUNDIFY_CLIENT_ID, client_key=HOUNDIFY_CLIENT_KEY))
    except sr.UnknownValueError:
  ...
  def _ibm(self, audio):
    ...
    try:
      print("IBM Speech to Text thinks you said " + self.recognizer.recognize_ibm(audio, username=IBM_USERNAME, password=IBM_PASSWORD))
    except sr.UnknownValueError:
